...
if __name__ == "__main__":
    project_test = {
        "m1": {
            ...
            "calls": {
                "calls/raw/wc_m1b_raw.json": "report/wc/Batch 38 call m1b.csv",
    ...
    for test_m in list_test:
        ...
        criteria_name = ["wrongNumber", "confirmSimWC"]
        ...
        for call in list_call:
            ...
            report = pd.read_csv(list_call[call])[
                ["File Name", "Criterion Name", "Expected Result"]
            ]
            call = call.split("/")[-1].replace(".json", "")
            mapping = {"Follow the structure of the call": "confirmSimWC"}
            report["Expected Result"] = (
                report["Expected Result"]
                .fillna("No")
                .apply(lambda x: x if str(x) != "Partially" else "No")
            )
            # print(report)
            for criteria in mapping:
                mini_score = report[report["Criterion Name"] == criteria]
                compare_to = result_df[["name", mapping[criteria]]]
                compare_to[mapping[criteria]] = compare_to[mapping[criteria]].apply(
                    lambda x: "yes" if "yes" in x else x
                )
                result_compare = pd.merge(
                    mini_score,
                    compare_to,
                    left_on="File Name",
                    right_on="name",
                )
                result_compare = result_compare.drop(columns="name")
                result_compare["Expected Result"] = result_compare[
                    "Expected Result"
                ].str.lower()
                print(criteria)
                result_compare_fail = result_compare[
                    result_compare["Expected Result"]
                    != result_compare[mapping[criteria]]
                ]
                print("acc:", 1 - len(result_compare_fail) / len(result_compare))
                print("pass:", len(result_compare) - len(result_compare_fail))
                print("fail:", len(result_compare_fail))
                print(result_compare_fail)
            print(call)
            if "wc" in call:
