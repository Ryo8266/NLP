...
if __name__ == "__main__":
    ...
    call_path = "calls/clean/fec_200_clean.json"
    label_path = "report/fec/FEC_Batch 200 call_v5.csv"

    calls = pd.read_json(call_path)
    ...
    labels = pd.read_csv(label_path)[["file_name", "criterion_name", "audit_result"]]
    labels["criterion_name"] = labels["criterion_name"].str.lower()
    labels["audit_result"] = labels["audit_result"].str.lower()

    labels = labels[labels["criterion_name"] == "call classify"].rename(
        columns={"audit_result": "label"}
    )
    ...
    with open(out_dir + bot_name + ".json", "w", encoding="utf8") as fo:
        print(bot_name)
